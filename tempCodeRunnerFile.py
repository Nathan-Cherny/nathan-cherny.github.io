...
Walks source files in a static site's git repo, looks up each file's real
last-commit date, maps the file to its published URL, and writes sitemap.xml.

USAGE:
    python3 generate_sitemap.py

Run this from inside your git repo (or set REPO_DIR below).
"""
...
REPO_DIR = Path(__file__).parent          # root of your git repo
SOURCE_DIR = REPO_DIR                     # folder containing source files (e.g. REPO_DIR / "content")
BASE_URL = "https://xsotec.com"           # your site's domain, no trailing slash

# File extensions to include as pages. Adjust to your setup
# (e.g. add ".html" if you hand-write HTML, or ".md" for markdown source)
SOURCE_EXTENSIONS = {".html", ".md"}

# Files/folders to exclude entirely (build output, partials, config, etc.)
EXCLUDE_DIRS = {".git", "node_modules", "_site", "dist", "build", "templates", "includes"}
EXCLUDE_FILES = {"404.html", "404.md"}    # error pages should never be in the sitemap

OUTPUT_FILE = REPO_DIR / "sitemap.xml"

# ---------------------------------------------------------------------------
# CUSTOMIZE: map a source file path -> published URL path
# This depends entirely on your site generator / folder structure.
# Example below assumes: about.md -> /about/  and index.md -> /
# Adjust this function to match how your build actually resolves URLs.
# ---------------------------------------------------------------------------

def file_to_url_path(source_file: Path) -> str:
    ...
    if stem == "index":
        parts = parts[:-1]  # index.md at any level -> that folder's root
...
def get_last_commit_date(file_path: Path) -> str | None:
    """Return the ISO 8601 commit date of the last commit touching this file,
    or None if the file has no git history (e.g. uncommitted/new file)."""
    try:
        ...
        print(f"  git error for {file_path}: {e.stderr.strip()}", file=sys.stderr)
        return None
...
def collect_source_files():
    ...
    for path in SOURCE_DIR.rglob("*"):
        ...
        if path.name in EXCLUDE_FILES:
            continue
        if any(part in EXCLUDE_DIRS for part in path.relative_to(REPO_DIR).parts):
            continue
        files.append(path)
    return sorted(files)
...
def build_sitemap_entries():
    entries = []
    for source_file in collect_source_files():
        url_path = file_to_url_path(source_file)
        lastmod = get_last_commit_date(source_file)

        if lastmod is None:
            print(f"  WARNING: no git history for {source_file}, skipping lastmod")
...
def write_sitemap(entries):
    ...
    print(f"\nWrote {len(entries)} URLs to {OUTPUT_FILE}")


def make_sitemap():
    print(f"Scanning {SOURCE_DIR} for source files...")
    entries = build_sitemap_entries()

    if not entries:
        print("No source files found. Check SOURCE_DIR / SOURCE_EXTENSIONS config.", file=sys.stderr)
        sys.exit(1)
